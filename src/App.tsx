...
import { mockInstitutions, mockStocks, defaultPreferences } from './data/mockData';
...
function App() {
  ...
  const [incomeTarget, setIncomeTarget] = useState(defaultPreferences.incomeTarget);
  const [frequency, setFrequency] = useState(defaultPreferences.frequency);
  const [willingSell, setWillingSell] = useState(defaultPreferences.willingSell);
  const [strikePrice, setStrikePrice] = useState(defaultPreferences.strikePrice);
    ...
    <div className="h-screen flex flex-col bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-gray-900 via-[#091428] to-black text-white">
      ...
      <div className="relative flex-1 flex flex-col max-w-[1400px] mx-auto w-full p-4 overflow-hidden">
        ...
        <header className="flex-none text-center py-4">
          ...
          <p className="text-sm sm:text-base text-white/60">Generate consistent income from your stock portfolio</p>
        </header>
